...
const SubmitDesign = () => {
  ...
  const handleSubmit = async () => {
    if (!user) {
      toast({ title: "Please sign in", description: "You need to sign in to submit a design.", variant: "destructive" });
      return;
    }
    if (!file || !form.creator_name.trim() || !form.email.trim() || !form.description.trim()) {
      toast({ title: "Missing fields", description: "Please fill in all required fields and upload a model.", variant: "destructive" });
      return;
    }
            ...
            <div className="space-y-4">
              <Card>
                ...
                <CardContent>
                  ...
                  <input id="design-file-input" type="file" accept={ACCEPTED_EXTENSIONS} onChange={handleFileChange} className="hidden" />
                </CardContent>
              </Card>
              {previewUrl && (
                ...
                {!user ? (
                  <div className="py-8 text-center">
                    <p className="text-muted-foreground">
                      Please <Link to="/auth" className="text-primary hover:underline">sign in</Link> to submit a design.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input value={form.creator_name} onChange={(e) => setForm({ ...form, creator_name: e.target.value })} placeholder="Your name or studio" maxLength={100} />
                    </div>
                    <div>
                      <Label>Email *</Label>
                    ...
                    <div>
                      ...
                      <Input value={form.portfolio_url} onChange={(e) => setForm({ ...form, portfolio_url: e.target.value })} placeholder="https://your-portfolio.com" maxLength={500} />
                    </div>
                    <div>
                      <Label>Description *</Label>
                      <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Tell us about your design..." rows={4} maxLength={2000} />
                    </div>
                    <Button onClick={handleSubmit} disabled={submitting || !file} className="w-full font-display uppercase tracking-wider">
