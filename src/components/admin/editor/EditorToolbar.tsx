...
import {
  ...
  X, AlertCircle, Settings2, Trash2,
} from "lucide-react";
...
import { useVisualEditor, pageToEditorKey, pageDisplayTitle } from "@/contexts/VisualEditorContext";
        ...
        >
          ...
          <SelectContent>
            ...
            {globalPages.length > 0 && (
              <SelectGroup>
                <SelectLabel className="text-[10px] uppercase tracking-wider text-muted-foreground">Global</SelectLabel>
                {globalPages.map(p => (
                  <SelectItem key={p.id} value={pageToEditorKey(p)}>{pageDisplayTitle(p)}</SelectItem>
                ))}
              </SelectGroup>
