...
import { Flame, Eye, ShoppingBag, Clock, TrendingUp } from "lucide-react";
...
export default function ProductFOMOBar({ productId, stock }: Props) {
  ...
  if (viewingNow >= 5) {
    signals.push({
      ...
      text: `${viewingNow} people viewing right now`,
  ...
  if (weeklyPurchases >= 2) {
    signals.push({
      ...
      text: `Purchased ${weeklyPurchases}× this week`,
      highlight: true,
    });
  } else if (lastPurchased) {
    signals.push({
      icon: <ShoppingBag className="h-3.5 w-3.5" />,
      text: `Last bought ${formatRelativePurchaseTime(lastPurchased)}`,
    });
  }
  ...
  if (stock > 0 && stock <= 10) {
    signals.push({
      ...
      text: stock <= 3 ? `Only ${stock} left!` : `${stock} remaining in stock`,
      highlight: stock <= 3,
    });
  }
    ...
    >
      {signals.map((signal, i) => (
        <motion.div
          ...
          className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs backdrop-blur-sm ${
            signal.highlight
