...
const OrderTracking = () => {
  ...
  if (!order) return (
    <div className="flex flex-col items-center justify-center py-24">
      <h1 className="mb-4 font-display text-2xl font-bold uppercase text-foreground">Order Not Found</h1>
      ...
      <div className="container max-w-3xl">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="mb-6 flex items-center justify-between">
            <div>
              <h1 className="font-display text-3xl font-bold uppercase text-foreground">Order #{order.id.slice(0, 8)}</h1>
              <p className="text-sm text-muted-foreground">{new Date(order.created_at).toLocaleDateString()}</p>
            </div>
            <Badge className={`font-display uppercase ${statusColors[order.status]}`}>
              <StatusIcon className="mr-1 h-4 w-4" /> {order.status}
            </Badge>
          </div>
                    ...
                    return (
                      <div key={s} className="flex flex-1 items-center">
                        <div className={`flex flex-col items-center ${reached ? "text-primary" : "text-muted-foreground"}`}>
                          ...
                          <span className="mt-1 font-display text-xs uppercase">{s}</span>
                        </div>
              ...
              <CardContent className="flex items-center justify-between p-4">
                ...
                {order.tracking_url && (
                  <a href={order.tracking_url} target="_blank" rel="noopener noreferrer">
                    <Button variant="outline" size="sm" className="font-display uppercase tracking-wider">Track Package</Button>
                  </a>
                )}
              </CardContent>
            </Card>
          ...
          {/* Order Items */}
          <Card className="mb-6">
            <CardHeader><CardTitle className="font-display uppercase">Items</CardTitle></CardHeader>
            <CardContent>
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex items-center justify-between border-b border-border pb-3 last:border-0 last:pb-0">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Qty: {item.quantity} × {Number(item.unit_price).toFixed(2)} kr</p>
                    </div>
                    <span className="font-display font-bold text-primary">{Number(item.total_price).toFixed(2)} kr</span>
                  </div>
                ))}
              </div>
              <div className="mt-4 space-y-1 border-t border-border pt-4">
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Subtotal</span><span>{Number(order.subtotal).toFixed(2)} kr</span></div>
                <div className="flex justify-between text-sm"><span className="text-muted-foreground">Shipping</span><span>{Number(order.shipping_cost).toFixed(2)} kr</span></div>
                <div className="flex justify-between font-display font-bold text-lg"><span>Total</span><span className="text-primary">{Number(order.total).toFixed(2)} kr</span></div>
              </div>
            </CardContent>
          </Card>
          ...
          {history.length > 0 && (
            <Card>
              <CardHeader><CardTitle className="font-display uppercase">Status History</CardTitle></CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {history.map((h, i) => (
                    <div key={h.id} className="flex gap-4">
                      ...
                      <div className="pb-4">
                        <p className="font-display text-sm font-semibold uppercase text-card-foreground">{h.status}</p>
